const DataPizza = [
  {
    name: "Nordique",
    category: "Exotique",
    id: 1,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/nord.36477c6ab92657b8bc026c32220a4e23.1.jpg",
    phrase: "Retiens ta respiration !",
  },
  {
    name: "Pepperoni",
    category: "Carnivore",
    id: 2,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/pepperoni.313df36c8b232421470453bbb60201dd.1.jpg",
    phrase:
      "Les daltoniens choisissent leurs poivrons au hasard. Chez Mood, on ne fait pas de discrimination : on vous met des rouges, des jaunes et des verts sans distinction !",
  },
  {
    name: "Chèvre Miel",
    category: "Fromage",
    id: 3,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/chevre-miel.8104a0acdb16d18dac2b8ad574aa92f2.1.jpg",
    phrase: "Qui manie le miel s en lèche les doigts.",
  },
  {
    name: "Chicken Barbecue",
    category: "Poulet",
    id: 4,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/chicken-bbq.3a33b97a501154997e8a3167eca3e8ed.1.jpg",
    phrase:
      "Il va falloir courir, mon petit poulet, si tu ne veux pas finir à la casserole !",
  },
  {
    name: "Texane Barbecue",
    category: "Carnivore",
    id: 5,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/texane-bbq.fc5bbf2798477052fb3a288a09b0aba9.1.jpg",
    phrase:
      "Hey Cow-Boy ! Il ne faut pas partir à la conquête de l Ouest le ventre vide ! Prends-donc un peu de cette délicieuse pizza Texane !",
  },
  {
    name: "Spicy Hot One",
    category: "Exotique",
    id: 6,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/spicy-hot-one.5752e3b2035e831e7f98e0fe97fdbe09.1.jpg",
    phrase:
      "Un peu de piment pour faire genre que tu ne pleures pas pour ce film... si, si, on t as grillé !",
  },
  {
    name: "Provençale",
    category: "Végétarienne",
    id: 7,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/provencale.b2790760a7c41d37418000cdebaab0d9.1.jpg",
    phrase: "Alors comme ça, la sardine, elle a bouché le port de Marseille ? ",
  },
  {
    name: "Hawaïenne jambon",
    category: "Exotique",
    id: 8,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/hawaienne.6cf6b87d02e3b45e3702023191b5158e.1.jpg",
    phrase: "De l ananas sur une pizza ? C est bon, on ne t en veux pas ! ",
  },
  {
    name: "Kasbah Kebab",
    category: "Carnivore",
    id: 9,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/kasbah.c3aa509816d983a89fed7e99ea9f58af.1.jpg",
    phrase:
      'On a hésité entre te proposer "Rcok the Kasbah" et "Le silence des agneaux". Finalement, notre choix de film est bien plus sage !',
  },
  {
    name: "Montagnarde",
    category: "Fromage",
    id: 10,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/montagnarde.08fb304ab7943248500a62f78d039db7.1.jpg",
    phrase: "Puisque la montagne ne vient pas à nous, allons à la montagne.",
  },
  {
    name: "Orientale",
    category: "Exotique",
    id: 11,
    image:
      "https://api.pizzahut.io/v1/content/fr-fr/fr-1/images/pizza/orientale.aa0e75b013460d6599bc7cb9c53d5a16.1.jpg",
    phrase:
      '"Le respect est le lien de l amitié." disait un sage oriental. Nous, on trouve que la pizza aussi !',
  },
  {
    name: "Pomodoro e burrata",
    category: "Fromage",
    id: 12,
    image:
      "https://medias.delarte.fr/media/sys_master/images/h50/heb/8855997087774.png",
    phrase:
      "La tomate il faut la peler. Oui mais comment l appeler autrement que tomate ? On est marrants, chez Mood. Bonne dégustation !",
  },
  {
    name: "Carbonara",
    category: "Fromage",
    id: 13,
    image:
      "https://medias.delarte.fr/media/sys_master/images/he3/hd9/8855991975966.png",
    phrase:
      "La carbonara, ce n est pas dans les pâtes, d habitude ? Ah bah, chez Mood, on innove, on te propose d en faire une pizza !",
  },
  {
    name: "3 Salumi",
    category: "Carnivore",
    id: 14,
    image:
      "https://medias.delarte.fr/media/sys_master/images/h70/h97/8855991189534.png",
    phrase:
      '"Toutes les bonnes choses ont une fin, sauf le saucisson qui en a deux !" Et toi, es-tu pressé de terminer ton film ou ta pizza ?',
  },
  {
    name: "Vezuvio",
    category: "Poulet",
    id: 15,
    image:
      "https://medias.delarte.fr/media/sys_master/images/hfd/hac/8855967531038.png",
    phrase:
      "L originalité est délicieuse, mais malheureusement, elle ne se commande pas aussi facilement qu une pizza.",
  },
  {
    name: "5 Formaggi",
    category: "Fromage",
    id: 16,
    image:
      "https://medias.delarte.fr/media/sys_master/images/h7f/h46/8855998267422.png",
    phrase:
      "Mon bon Monsieur, Apprenez que tout flatteur. Vit aux dépens de celui qui l écoute : Cette leçon vaut bien un fromage, sans doute.",
  },
  {
    name: "Speciale di Pizzaiolo",
    category: "Carnivore",
    id: 17,
    image:
      "https://medias.delarte.fr/media/sys_master/images/h44/hed/8855968710686.png",
    phrase:
      "On s en fiche du caviar, nous, on aime la pizza ! C est le chef qui l a dit, pas nous !",
  },
  {
    name: "Salmone",
    category: "Exotique",
    id: 18,
    image:
      "https://medias.delarte.fr/media/sys_master/images/h53/hf9/8855983718430.png",
    phrase:
      '"Le saumon vit dans des boîtes en fer-blanc, d où il ne sort que le dimanche soir quand des amis arrivent à l improviste." disait un célèbre philosophe. Chez Mood, on a pensé à cet improviste !',
  },
  {
    name: "Regina",
    category: "Poulet",
    id: 19,
    image:
      "https://medias.delarte.fr/media/sys_master/images/h8e/h43/8855977033758.png",
    phrase: "L argent ne fait pas le bonheur. La pizza, oui.",
  },
  {
    name: "Bianca",
    category: "Fromage",
    id: 20,
    image:
      "https://medias.delarte.fr/media/sys_master/images/heb/hc3/8863126618142.png",
    phrase: "Vivons d amour et de pizza !",
  },
  {
    name: "Diavola",
    category: "Poulet",
    id: 21,
    image:
      "https://medias.delarte.fr/media/sys_master/images/hc6/hfc/8864554975262.png",
    phrase:
      "50% des mariages se terminent par un divorce. 100% des livraisons de pizza se terminent pas un bonheur pur. CQFD.",
  },
  {
    name: "Del Mare",
    category: "Exotique",
    id: 22,
    image:
      "https://medias.delarte.fr/media/sys_master/images/hc4/h41/8863150997534.png",
    phrase:
      "C est pas l homme qui prend la mer, c est la mer qui prend l homme ! Renaud aurait aussi dû emmener une pizza. ",
  },
  {
    name: "Margarita",
    category: "Fromage",
    id: 23,
    image:
      "https://medias.delarte.fr/media/sys_master/images/h93/ha5/8863144312862.png",
    phrase: "Libert, égalité, pizza devant la télé !",
  },
  {
    name: "Carciofi",
    category: "Végétarien",
    id: 24,
    image:
      "https://medias.delarte.fr/media/sys_master/images/hfa/he5/8863139987486.png",
    phrase:
      'Vous serez tellement plongé dans ce film que vous allez provoquer un "pizzaccident" : manger une pizza entière par accident ! ',
  },
];
